...
def create_step0003_tsv_from_step0002_tsv(psz_step0002_tsv_path: str) -> str:
    ...
    obj_step0002_tsv_path: Path = Path(psz_step0002_tsv_path)
    psz_step_stem: str = obj_step0002_tsv_path.stem
    psz_output_stem: str = psz_step_stem[:-9] if psz_step_stem.endswith("_step0002") else psz_step_stem
    obj_step0003_tsv_path: Path = obj_step0002_tsv_path.with_name(f"{psz_output_stem}_step0003.tsv")

    with obj_step0002_tsv_path.open(mode="r", encoding="utf-8", newline="") as obj_input_file:
        list_input_lines: list[str] = [psz_line.rstrip("\r\n") for psz_line in obj_input_file]

    with obj_step0003_tsv_path.open(mode="w", encoding="utf-8", newline="\r\n") as obj_output_file:
        for psz_line in list_input_lines:
            list_columns: list[str] = psz_line.split("\t")
            list_remaining_columns: list[str] = list_columns[10:] if len(list_columns) > 10 else []
            obj_output_file.write("\t".join(list_remaining_columns) + "\n")

    return str(obj_step0003_tsv_path)
